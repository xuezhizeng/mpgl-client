...
/**
 * 一个收缩面板
 * @deprecated 与相关皮肤冲突,暂时废弃
 * @author 廖陈特
 *
 */
public class JGroupPanel extends JPanel {
	
	private static final long serialVersionUID = -8204664888200137649L;
	...
	public JGroupPanel(List<MenuVo> list) {
		initComponents();
		createDefaultGroup(list);
	}
	...
	private void createDefaultGroup(List<MenuVo> list) {
		for (int i = 0; i < list.size(); i++) {
			MenuVo menu = list.get(i);
			List<SubMenuVo> subList = menu.getList();
			insertGroup(i, menu.getName());
			for (int j = 0; j < subList.size(); j++) {
				SubMenuVo subMenu = subList.get(j);
				JButton button = new JButton(subMenu.getName());
				button.setActionCommand(subMenu.getCode());
				addMember(i, button);
				button.setPreferredSize(new Dimension(150, 40));
			}
			getGroup(i).setMemberGap(20, 5);
		}
	 ...
	 */
	public List<JGroupContainer> getGroup() {
		return (List<JGroupContainer>)groupList;
	}
	 ...
	 * @version 1.0
	 */
 public	class JGroupContainer extends JPanel {
