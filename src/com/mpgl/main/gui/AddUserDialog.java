...
public class AddUserDialog extends BaseDialog {
	...
	private static final long serialVersionUID = 811060349585208316L;
	 ...
	 */
	private void save() {
		if(!CommonUtil.isNotNull(this.textUsername.getText())) {
			JOptionPane.showMessageDialog(this.frame,
					Constant.MESSAGE.ERROR_ADDUSERNAME, Constant.MESSAGE.TITLE_ERROR,
		...
		userVo.setCreate_time(Constant.DEFAULT_DATE_FORMAT.format(new Date()));
		if (mainService.checkUserName(userVo)) {
			mainService.addUser(userVo);
			this.close();
		} else {
			JOptionPane.showMessageDialog(this.frame,
					Constant.MESSAGE.USER_REPEAT, Constant.MESSAGE.TITLE_ERROR,
					JOptionPane.ERROR_MESSAGE, Constant.iconError);
		}
	}
	...
	private void initForm() {
		...
		this.comboboxIsLock.setSelectedIndex(0);
		this.comboboxRole.setSelectedIndex(0);
	}
